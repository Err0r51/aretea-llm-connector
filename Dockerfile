...
# Stage 1: resolve + install deps into a venv, with cache/bind mounts for fast rebuilds.
...
# Install deps first (cached unless the lockfile changes), then the project.
...
CMD ["sh", "-c", "python -m aretea_drive_mcp.init_db && uvicorn aretea_drive_mcp.main:app --host 0.0.0.0 --port ${PORT:-8000}"]
